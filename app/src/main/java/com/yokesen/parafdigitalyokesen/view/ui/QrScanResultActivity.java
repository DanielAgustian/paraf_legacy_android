...
public class QrScanResultActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initLoadingComponent() {
        ...
        llEmpty = findViewById(R.id.emptyLL);
        tvMessageError = findViewById(R.id.messageError);
    }
...
//        }
    ...
    private void initData() {
        ...
        String qrCode = getIntent().getStringExtra("qrCode") ;
            Observable<GetQRSignModel> getDetails = apiInterface.GetSignQRData(token, qrCode);
            getDetails.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(this::onSuccess, this::onFailed);
    }

    private void onSuccess(GetQRSignModel model) {
        if(model != null){
            String message = model.getMessage();
            if(message == null || message.equals("")){
                setData(model.getHome());
            } else if (message.contains("input code")){
                qrCode = model.getHome().getQr_code();
    ...
    private void onFailed(Throwable throwable) {
        ...
        if(error.toLowerCase().contains("401")){
            //util.toastMisc(this, "You cannot open this document. You can oly open them from inside app or website, not by Qr Scan.");
            llEmpty.setVisibility(View.VISIBLE);
            llInfo.setVisibility(View.GONE);
            tvMessageError.setText("This document is private document. You are unathorized to open it.");
        }else{
    ...
    private void setData(SignatureDetailModel detailModel) {
        pd = util.makeQRCOde(detailModel.getQr_code());

        LinearLayout llPersonRespond = findViewById(R.id.llPersonRespond);
        ...
        tvCreatedBy.setText(detailModel.getTitle());
        ...
        tvInitiated.setText(detailModel.getDateCreated()+" | "+ detailModel.getTimeHour());

        TextView tvTitle = findViewById(R.id.tvDocName);
        tvTitle.setText(detailModel.getDocumentName());
        ...
        if(detailModel.getFile().equals("")){
            tvlink.setText(detailModel.getLink());
        }else{
            tvlink.setText(detailModel.getFile());
        }
    ...
    private void dialogPasscode(){
        Dialog dialogPasscode  = new Dialog(this);
        dialogPasscode.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialogPasscode.setContentView(R.layout.dialog_qr_passcode);
        dialogPasscode.setCancelable(true);
        dialogPasscode.show();
        Button btnContinue = dialogPasscode.findViewById(R.id.btnContinueRename);
        Button btnCancel = dialogPasscode.findViewById(R.id.btnCancelRename);
        EditText etPasscode = dialogPasscode.findViewById(R.id.etPasscodeDoc);
    ...
    private void doPasscode(String passcode) {
        Observable<GetQRSignModel> CheckPasscode = apiInterface.CheckQrPasscode(token, qrCode, passcode);
        CheckPasscode.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(this::onSuccessPasscode, this::onFailedPasscode);
    }

    private void onFailedPasscode(Throwable throwable) {
        util.toastMisc(this, "Wrong password. Please Try Again");
        dialogPasscode();
    }

    private void onSuccessPasscode(GetQRSignModel getQRSignModel) {
        if(getQRSignModel!=null){
            SignatureDetailModel model = getQRSignModel.getHome();
...
//
//                    data = util.dpToPx(data, QrScanResultActivity.this);
...
//    }
    ...
    public void initComponent(){
        ...
        llRegen.setVisibility(View.GONE);

        llSave  =findViewById(R.id.llSave);
        llSave.setVisibility(View.GONE);

        llDuplicate = findViewById(R.id.llDuplicate);
        llDuplicate.setVisibility(View.GONE);
